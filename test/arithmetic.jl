...
@testset "Vectorized arithmetic" begin
    ...
    @test dr .- Dates.Day(1) == repeat([b], 10)
    t1 = [
        Dates.Date(2009, 1, 1) Dates.Date(2009, 1, 2) Dates.Date(2009, 1, 3)
        Dates.Date(2009, 2, 1) Dates.Date(2009, 2, 2) Dates.Date(2009, 2, 3)
    ]
    t2 = [
        Dates.Date(2009, 1, 2) Dates.Date(2009, 2, 2) Dates.Date(2010, 1, 3)
        Dates.Date(2010, 2, 1) Dates.Date(2009, 3, 2) Dates.Date(2009, 2, 4)
    ]
    t3 =
        [Dates.DateTime(2009, 1, 1), Dates.DateTime(2009, 1, 2), Dates.DateTime(2009, 1, 3)]
    t4 = [
        Dates.DateTime(2009, 1, 1, 0, 0, 1),
        Dates.DateTime(2009, 1, 2, 0, 1),
        Dates.DateTime(2009, 1, 3, 1),
    ]
    t5 = [
        Dates.Time(0, 0, 0) Dates.Time(0, 0, 1) Dates.Time(0, 0, 2)
        Dates.Time(0, 1, 0) Dates.Time(0, 2, 0) Dates.Time(0, 3, 0)
    ]

    @testset "TimeType, Array{TimeType}" begin
        @test Dates.Date(2010, 1, 1) .- t1 == [
            Dates.Day(365) Dates.Day(364) Dates.Day(363)
            Dates.Day(334) Dates.Day(333) Dates.Day(332)
        ]
        @test t1 .- Dates.Date(2010, 1, 1) == [
            Dates.Day(-365) Dates.Day(-364) Dates.Day(-363)
            Dates.Day(-334) Dates.Day(-333) Dates.Day(-332)
        ]
        @test Dates.DateTime(2009, 1, 1) .- t3 == [
            Dates.Millisecond(0),
            Dates.Millisecond(-86400000),
            Dates.Millisecond(-172800000),
        ]
        @test t3 .- Dates.DateTime(2009, 1, 1) == [
            Dates.Millisecond(0),
            Dates.Millisecond(86400000),
            Dates.Millisecond(172800000),
        ]
        @test Dates.Time(2) .- t5 == [
            Dates.Nanosecond(7200000000000) Dates.Nanosecond(7199000000000) Dates.Nanosecond(7198000000000)
    ...
    @testset "GeneralPeriod, Array{TimeType}" begin
        @test Dates.Day(1) .+ t1 == [
            Dates.Date(2009, 1, 2) Dates.Date(2009, 1, 3) Dates.Date(2009, 1, 4)
            Dates.Date(2009, 2, 2) Dates.Date(2009, 2, 3) Dates.Date(2009, 2, 4)
        ]
        @test Dates.Hour(1) .+ t3 == [
            Dates.DateTime(2009, 1, 1, 1),
            Dates.DateTime(2009, 1, 2, 1),
            Dates.DateTime(2009, 1, 3, 1),
        ]
        @test t1 .+ Dates.Day(1) == [
            Dates.Date(2009, 1, 2) Dates.Date(2009, 1, 3) Dates.Date(2009, 1, 4)
        ...
        @test t3 .+ Dates.Hour(1) == [
            Dates.DateTime(2009, 1, 1, 1),
            Dates.DateTime(2009, 1, 2, 1),
            Dates.DateTime(2009, 1, 3, 1),
        ]

        @test (Dates.Month(1) + Dates.Day(1)) .+ t1 == [
            Dates.Date(2009, 2, 2) Dates.Date(2009, 2, 3) Dates.Date(2009, 2, 4)
            Dates.Date(2009, 3, 2) Dates.Date(2009, 3, 3) Dates.Date(2009, 3, 4)
        ]
        @test (Dates.Hour(1) + Dates.Minute(1)) .+ t3 == [
            Dates.DateTime(2009, 1, 1, 1, 1),
            Dates.DateTime(2009, 1, 2, 1, 1),
            Dates.DateTime(2009, 1, 3, 1, 1),
        ]
        @test t1 .+ (Dates.Month(1) + Dates.Day(1)) == [
            Dates.Date(2009, 2, 2) Dates.Date(2009, 2, 3) Dates.Date(2009, 2, 4)
            Dates.Date(2009, 3, 2) Dates.Date(2009, 3, 3) Dates.Date(2009, 3, 4)
        ]
        @test t3 .+ (Dates.Hour(1) + Dates.Minute(1)) == [
            Dates.DateTime(2009, 1, 1, 1, 1),
            Dates.DateTime(2009, 1, 2, 1, 1),
            Dates.DateTime(2009, 1, 3, 1, 1),
        ]
        ...
        @test t1 .- Dates.Day(1) == [
            Dates.Date(2008, 12, 31) Dates.Date(2009, 1, 1) Dates.Date(2009, 1, 2)
            Dates.Date(2009, 1, 31) Dates.Date(2009, 2, 1) Dates.Date(2009, 2, 2)
        ]
        @test t3 .- Dates.Hour(1) == [
            Dates.DateTime(2008, 12, 31, 23),
            Dates.DateTime(2009, 1, 1, 23),
            Dates.DateTime(2009, 1, 2, 23),
        ]

        @test t1 .- (Dates.Month(1) + Dates.Day(1)) == [
            Dates.Date(2008, 11, 30) Dates.Date(2008, 12, 1) Dates.Date(2008, 12, 2)
            Dates.Date(2008, 12, 31) Dates.Date(2009, 1, 1) Dates.Date(2009, 1, 2)
        ]
        @test t3 .- (Dates.Hour(1) + Dates.Minute(1)) == [
            Dates.DateTime(2008, 12, 31, 22, 59),
            Dates.DateTime(2009, 1, 1, 22, 59),
            Dates.DateTime(2009, 1, 2, 22, 59),
        ]
    end
    @testset "#20205" begin
    ...
    @testset "deprecated" begin
        ...
        @test t5 + Dates.Hour(1) == [
            Dates.Time(1, 0, 0) Dates.Time(1, 0, 1) Dates.Time(1, 0, 2)
            Dates.Time(1, 1, 0) Dates.Time(1, 2, 0) Dates.Time(1, 3, 0)
        ]
        @test (Dates.Month(1) + Dates.Day(1)) + t1 == [
            Dates.Date(2009, 2, 2) Dates.Date(2009, 2, 3) Dates.Date(2009, 2, 4)
            Dates.Date(2009, 3, 2) Dates.Date(2009, 3, 3) Dates.Date(2009, 3, 4)
        ...
        @test t2 - t1 == [
            Dates.Day(1) Dates.Day(31) Dates.Day(365)
            Dates.Day(365) Dates.Day(28) Dates.Day(1)
        ]
        @test t4 - t3 == [
            Dates.Millisecond(1000),
            Dates.Millisecond(60000),
            Dates.Millisecond(3600000),
        ]
        @test (Dates.Date(2009, 1, 1):Dates.Week(1):Dates.Date(2009, 1, 21)) -
              (Dates.Date(2009, 1, 1):Dates.Day(1):Dates.Date(2009, 1, 3)) ==
              [Dates.Day(0), Dates.Day(6), Dates.Day(12)]
        @test (
            Dates.DateTime(2009, 1, 1, 1, 1, 1):Dates.Second(1):Dates.DateTime(
                2009,
                1,
                1,
                1,
            ...
            Dates.DateTime(2009, 1, 1, 1, 1):Dates.Second(1):Dates.DateTime(
                2009,
                1,
                1,
                1,
        ...
        ) == [Dates.Second(1), Dates.Second(1), Dates.Second(1)]
        @test_throws MethodError Dates.Day(1) - t1
        @test_throws MethodError Dates.Hour(1) - t3
        @test_throws MethodError (Dates.Month(1) + Dates.Day(1)) - t1
        @test_throws MethodError (Dates.Hour(1) + Dates.Minute(1)) - t3
