...
### truncation
Base.trunc(dt::DayDate, p::Type{Year}) = Date(UTD(totaldays(year(dt), 1, 1)))
Base.trunc(dt::DayDate, p::Type{Quarter}) = firstdayofquarter(dt)
Base.trunc(dt::DayDate, p::Type{Month}) = firstdayofmonth(dt)
Base.trunc(dt::DayDate, p::Type{Day}) = dt


"""
    trunc(dt::SimpleDate, ::Type{Period}) -> SimpleDate

Truncates the value of `dt` according to the provided `Period` type.

# Examples
```jldoctest
julia> trunc(ExtendedDates.DayDate("1996-01-02"), Dates.Month)
1996-01-01
```
"""
Dates.trunc(::SimpleDate, ::Type{Dates.Period})
