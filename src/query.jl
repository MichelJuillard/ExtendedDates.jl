...
### Days of the Week
"""
    dayofweek(dt::Dates.TimeType) -> Int64
...
```
"""
dayofweek(dt::DayDate) = dayofweek(days(dt))
...
"""
    dayabbr(dt::Dates.TimeType; locale="english") -> String
...
ismonday(dt::Dates.TimeType) = dayofweek(dt) == Mon
istuesday(dt::Dates.TimeType) = dayofweek(dt) == Tue
iswednesday(dt::Dates.TimeType) = dayofweek(dt) == Wed
isthursday(dt::Dates.TimeType) = dayofweek(dt) == Thu
isfriday(dt::Dates.TimeType) = dayofweek(dt) == Fri
issaturday(dt::Dates.TimeType) = dayofweek(dt) == Sat
issunday(dt::Dates.TimeType) = dayofweek(dt) == Sun

# i.e. 1st Monday? 2nd Monday? 3rd Wednesday? 5th Sunday?
"""
    dayofweekofmonth(dt::Dates.TimeType) -> Int
...
```
"""
function dayofweekofmonth(dt::Dates.TimeType)
    d = day(dt)
...
"""
    daysofweekinmonth(dt::Dates.TimeType) -> Int
